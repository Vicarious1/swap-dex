...
import { ButtonVariant, StoreState, Token, TokenBalance, Wallet, Web3State } from '../../../util/types';
...
class IEOWalletBalance extends React.Component<Props, State> {
    ...
    private readonly _getWalletContent = () => {
        ...
        if (baseTokenBalance && quoteTokenBalance) {
            ...
            content = (
                <>
                    <LabelWrapper>
                        <Label>{tokenSymbolToDisplayString(baseTokenBalance.token.symbol)}</Label>
...
const mapStateToProps = (state: StoreState): StateProps => {
    return {
        ...
        baseToken: getBaseTokenIEO(state) as Token,
